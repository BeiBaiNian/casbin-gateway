...
// config.yaml is the user's file, so the observer is enabled by splicing one
// list item into the text rather than round-tripping the document through a
// YAML marshaller, which would rewrite every line. Reads still go through the
// real parser, and every edit is re-parsed before it is committed.
...
func writeHermesPluginMembership(configPath string, enabled bool) error {
	...
	// Removing the observer can empty a config.yaml that only ever held the
	// block Patch appended, so Unpatch takes that file away with it.
	if !enabled && len(bytes.TrimSpace(updated)) == 0 {
		if err := os.Remove(configPath); err != nil && !os.IsNotExist(err) {
			return err
		}
		return nil
	}
	return writeFileAtomic(configPath, updated, 0o600)
}

// writeFileAtomic replaces path through a temporary file in the same directory,
// so an interrupted write can never truncate a config Gateway does not back up.
func writeFileAtomic(path string, data []byte, perm os.FileMode) error {
	...
	temp, err := os.CreateTemp(filepath.Dir(path), "."+filepath.Base(path)+".gateway-*")
	if err != nil {
		return err
	}
	name := temp.Name()
...
func setHermesPluginListMember(data []byte, key string, want bool) ([]byte, bool, error) {
	...
	if !want {
		...
		if key == "enabled" {
			document.pruneEmptyBlock(plugins, keyLine, childIndent)
		}
		return document.bytes(), true, nil
	}
	if existing >= 0 {
...
type yamlLines struct {
	lines []string
	// carriageReturn is what a line Gateway writes ends with. Existing lines
	// keep whichever ending they already had, so a file with mixed endings is
	// never normalized behind the user's back.
...
func newYAMLLines(data []byte) *yamlLines {
	...
	return &yamlLines{lines: strings.Split(string(data), "\n"), carriageReturn: carriageReturn}
}

func (y *yamlLines) bytes() []byte {
	return []byte(strings.Join(y.lines, "\n"))
}

func (y *yamlLines) insert(at int, lines ...string) {
	written := make([]string, len(lines))
	for index, line := range lines {
		written[index] = line + y.carriageReturn
	}
	y.lines = append(y.lines[:at:at], append(written, y.lines[at:]...)...)
}
...
// pruneEmptyBlock drops a plugins.<key> left holding nothing by the removal of
// Gateway's entry, then plugins itself, then the blank line that separated an
// appended block from the content above it. Comments count as content, so a
// block the user annotated is kept.
func (y *yamlLines) pruneEmptyBlock(pluginsLine, keyLine, childIndent int) {
	// Items may sit at the key's own indent, where the block under it looks
	// empty, so the sequence decides rather than the indentation.
	if len(y.sequenceItems(keyLine, childIndent)) > 0 {
		return
	}
	if y.hasText(keyLine+1, y.blockEnd(childIndent, keyLine+1)) {
		return
	}
	y.remove(keyLine)
	if y.hasText(pluginsLine+1, y.blockEnd(0, pluginsLine+1)) {
		return
	}
	y.remove(pluginsLine)
	if pluginsLine > 0 && strings.TrimSpace(y.lines[pluginsLine-1]) == "" &&
		!y.hasText(pluginsLine, len(y.lines)) {
		y.remove(pluginsLine - 1)
	}
}

func (y *yamlLines) hasText(start, end int) bool {
	for index := start; index < end && index < len(y.lines); index++ {
		if strings.TrimSpace(y.lines[index]) != "" {
			return true
		}
	}
	return false
...
func yamlSequenceItem(line string) (string, int, bool) {
	...
	rest := strings.TrimRight(line[indent:], "\r")
