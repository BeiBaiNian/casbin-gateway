...
export function getChannels(owner, page = "", pageSize = "") {
  return request(`/api/get-channels?owner=${encodeURIComponent(owner)}&p=${page}&pageSize=${pageSize}`);
}
...
export function testChannel(owner, name) {
  return request(`/api/test-channel?id=${encodeURIComponent(owner)}/${encodeURIComponent(name)}`, "POST", {
    owner: owner,
    name: name,
  });
}
