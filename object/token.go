...
// GetTokenBySecretKeyHash finds a token whose bcrypt hash matches the provided key.
func GetTokenBySecretKeyHash(providedKey string) (*Token, error) {
	tokens := []*Token{}
	err := ormer.Engine.Where("status = ?", "enabled").Find(&tokens)
	if err != nil {
		return nil, err
	}
	for _, token := range tokens {
		if token.SecretKeyHash == "" {
			continue
		}
		err := bcrypt.CompareHashAndPassword([]byte(token.SecretKeyHash), []byte(providedKey))
		if err == nil {
			return token, nil
		}
	}
	return nil, fmt.Errorf("token not found for provided key")
}

// GetTokenBySecretKeyHashExact looks up a token by hash and returns it regardless of status.
func GetTokenBySecretKeyHashExact(hash string) (*Token, error) {
	token := &Token{SecretKeyHash: hash}
	ok, err := ormer.Engine.Where("secret_key_hash = ?", hash).Get(token)
	if err != nil {
		return nil, err
	}
	if !ok {
...
// BuildTokenDisplayId builds an owner/name display id from a token.
func BuildTokenDisplayId(owner, name string) string {
	return fmt.Sprintf("%s/%s", owner, name)
}

// ParseTokenIdFromDisplay extracts owner and name from a display id.
func ParseTokenIdFromDisplay(id string) (string, string) {
